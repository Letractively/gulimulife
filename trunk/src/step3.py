...
class GulimuLife(webapp.RequestHandler):
    ...
    def get(self):
        ...
        self.response.out.write('<li><a href="%s">Google Documents</a></li>' % (
            client.GenerateAuthSubURL(
                next_url,
                ('https://www.google.com/calendar/feeds/',), secure=False, session=True)))
...
def main():
    application = webapp.WSGIApplication([('/.*', GulimuLife), ], debug=True)
